...
//  Marble
//
//  Created by Mac Bellingrath on 8/9/15.
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func loadLevel() {
        if let levelPath = NSBundle.mainBundle().pathForResource("level1", ofType: "txt"){
            do {
                let levelString = try NSString(contentsOfFile: levelPath, encoding: NSUTF8StringEncoding)
...
//    
//    func configureLayer(name: String, circle: Bool, position: CGPoint) {
//        
...
//    }
    ...
    func playerCollidedWithNode(node: SKNode ){
        if node.name == "vortex" {
            ...
            let scale = SKAction.scaleTo(0.001, duration: 0.25)
